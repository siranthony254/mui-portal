...
export default async function SettingsPage() {
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6 px-4">
      <div className="page-header"><h1 className="page-title text-lg sm:text-xl">Settings</h1></div>
      <div className="card p-4 sm:p-5">
        <p className="section-title text-sm">Platform architecture</p>
      ...
      <div className="card p-4 sm:p-5">
        <p className="section-title text-sm">External management</p>
        <div className="space-y-2">
          {[{label:'Sanity Studio',desc:'Add content blocks, courses, announcements',href:`https://sanity.io/manage/personal/project/${process.env.NEXT_PUBLIC_SANITY_PROJECT_ID}`},{label:'Supabase Dashboard',desc:'View database, auth users, run queries',href:'https://supabase.com/dashboard'},{label:'Vercel Dashboard',desc:'Deployment logs, env variables',href:'https://vercel.com/dashboard'}].map(link=>(
            <a key={link.label} href={link.href} target="_blank" rel="noopener noreferrer" className="flex items-center justify-between p-3 rounded-lg border border-gray-100 hover:border-teal-200 hover:bg-teal-50 transition-colors group">
              <div className="min-w-0"><p className="text-sm font-medium text-gray-900 group-hover:text-teal-700 truncate">{link.label}</p><p className="text-xs text-gray-400 mt-0.5 truncate">{link.desc}</p></div>
              <span className="text-gray-300 group-hover:text-teal-500 text-lg flex-shrink-0">→</span>
            </a>
          ))}
        </div>
      </div>

      <div className="card p-4 sm:p-5 bg-gray-900 text-white border-none shadow-xl relative overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          <h3 className="text-base sm:text-lg font-black mb-2">Platform Audit Logs</h3>
          <p className="text-xs text-gray-400 leading-relaxed mb-4 sm:mb-6 max-w-sm">
